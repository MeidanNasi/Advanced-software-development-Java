...
public class MatrixSearchable implements Searchable<Point> {
	...
	// constructor
	MatrixSearchable(double[][] matrix, Point source, Point goal, int rows, int col){
		this.matrix = matrix;
	...
	public String backTrace(State<Point> goal) {
		...
		trace.deleteCharAt(trace.length()-1);// Extra comma needs to be deleted
		return trace.toString();
	}

	private boolean isBound(int x, int y){
		return x >= 0 && x < row && y >= 0 && y < columns;
	}
