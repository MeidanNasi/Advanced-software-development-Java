...
public class BFS<T> implements Searcher<T> {
	...
	// add and pop Methods
	private void addToOpenList(State s) { openList.add(s); }
	private State<T> popOpenList() { return (openList.poll()); }
	private void updatedToBetterPath(State<T> state){ //pop and check the current openlist for changes, openlist now contain the best path
